...
\begin{document}
	...
	Hadoop MapReduce is a programming model for large-scale data processing. With the high demand of computing power, computer architecture has evolved over the years from serial computing to parallel computing where tasks are distributed and executed simultaneously across multiple processors within the same computer. Now with MapReduce, instead of using one server instance with multi-processing power, we will use multiple server instances with multi-processing power - this is called a distributed computing system.
	...
	A MapReduce framework consists of two workers: the Mapper and the Reducer. MapReduce uses the divide-and-conquer \cite{Divide-and-Conquer} technique where the input is divided into a set of small tasks and each task is identified by a key-value pair. The key serves as a task ID and the value as the task output. Each task is then processed and executed by the mapper and the outputs are processed and merged by the reducer \cite{MapReduce}.
