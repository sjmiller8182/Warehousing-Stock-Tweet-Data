...
\IEEEPARstart{D}{ata} in the twenty-first century is expanding in volumes
 at \textbf{large} rates. 
\textbf{E}very additional source of data that can act as a
 medium for data communication \textbf{may contain} useful information, which,
...
However, with the increasing ability to capture and store data from many
 ...
 likewise an increasing issue when it comes time to access and use \textbf{the data},
...
Developing a database \textbf{with the Hadoop ecosystem}, our team has built a data
...
\textbf{*I removed the part about parallelism because it is discussed later*}



Motivated \textbf{by the opportunity within "Big Data" to store and structure disparate data,
...
be managed and accessed within a network of servers \textbf{(a compute cluster)}
...
which assigns nodes for ``mapping'' and ``reducing'' \textbf{processes by applying}  
...
Quantitative data was gathered from a \textbf{stock market} data vendor through an
 Application Programming Interface (API) on 15-minute intervals using \textbf{the} R
 programming language. 
Qualitative data was gathered and processed through a Twitter API
 using \textbf{the} Python programming language. 
The data \textbf{was} structured and stored \textbf{in} a Hive data warehouse, 
 \textbf{which was} created and managed using Hive Query Language (HQL).
R will be used via Open Database Connectivity (ODBC) to access and
 build proof-of-concept models using the data. 
Once \textbf{the data warehouse is developed},
...
In order to provide manageable storage repositories that scale well to size and
 data diversity, we have implemented \textbf{this project} using \textbf{Simple Storage Service (S3)
 and Elastic MapReduce (EMR) from Amazon Web Services (AWS)}. 
S3 and EMR are designed for large sets of data. 
Therefore, integrity of data and ingestion systems are \textbf{to manage}. 
...
Performance \textbf{of the data warehouse} is based on \textbf{time} taken to process 
\textbf{the data collected for this project} 
