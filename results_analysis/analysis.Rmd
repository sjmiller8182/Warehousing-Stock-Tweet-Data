...
```{r, anova}
analysis.aov <- data %>% 
  aov(time ~ schema + Error(schema/block_size), data = .)
...
```{r, F_p}
F.stat <- 
  aov.summ$`Error: schema`[[1]]['Mean Sq'][[1]] / aov.summ$`Error: schema:block_size`[[1]]['Mean Sq'][[1]]

df1 <- aov.summ$`Error: schema`[[1]]['Df'][[1]]
df2 <- aov.summ$`Error: schema:block_size`[[1]]['Df'][[1]]
