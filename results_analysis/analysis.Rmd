...
grouped <- data %>% 
  group_by(server_num,schema, block_size) %>% 
...
```{r}
ggplot(data = grouped, aes(y = mean_time, x = block_size, color = schema:server_num)) +
  geom_point() +
  ...
  ggtitle('Query Time for Schemas and MapReduce Block Size Faceted by Cluster Size')
```
